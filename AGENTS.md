...
- `server.tf`: the Always Free compute instance from `github.com/clonecademy/terraform-oci-module-vps`, placed in the public subnet. SSH ingress is an NSG managed by the module, restricted to `ssh_ingress_cidrs` (deliberately no default). Tailscale is installed by hand, not through cloud-init.
  - Terraform only provisions the default `ubuntu` admin user (via `metadata.ssh_authorized_keys`). A second user, `kar`, with full `(ALL:ALL)` sudo and membership in `docker`, was created by hand during server housewarming and is not managed by Terraform or the module — don't assume it exists on a freshly created instance, or that recreating the instance preserves it. Password auth for `ubuntu` was disabled by hand as part of that same housewarming; only key auth and `kar`'s password work.
  - App services (e.g. Dokploy) run behind ports 80/443 opened by `web_ingress = true`; the NSG deliberately does not open other app ports (e.g. Dokploy's dashboard on 3000). Keep host-level `ufw` rules matching that intent — don't open ports in `ufw` that aren't meant to be reachable from the internet, since it sits behind the same public IP as the NSG. Use an SSH tunnel or Tailscale for direct access to a port that isn't meant to be public.
